...
設定画面の**ポーズ許容時間**は、文の途中でどれだけ間を空けても発話が終了と見なされないかを決めます(0.3〜2.0 秒、既定 0.5 秒)。考えながら話すときや数字を口述するときに途切れる場合は長くしてください。この値はパイプライン構築時に組み込まれるため、オーバーレイ実行中に変更するとモデルが再読み込みされます。

**ディクテーションの整形**(実験的、既定はオフ)は、オンデバイス LLM で
文字起こしの句読点を整え、フィラーを取り除きます。初回利用時に
SmolLM2-360M-Instruct(374 MB、Apache-2.0)をダウンロードします。出力が話した
内容から逸れた場合(文章の創作、整形ではなく応答など)は、未加工の文字起こしを
そのまま挿入します。バブルを長押しすると、設定を変えずにその 1 回だけ整形できます。

設定画面では**音声モデル**も選べます — Parakeet TDT(114 言語、ストリーミング
部分結果)または Whisper Small(4 言語、発話ごとに終了後デコード、ダウンロード
は小さい)。加えて、電話機ではなく**Bluetooth ヘッドセット**から録音できます。
ヘッドセットのマイクは帯域が狭いことが多いため既定はオフで、ヘッドセット未接続
時は本体マイクにフォールバックします。
