...
설정 화면의 **일시정지 허용 시간**은 문장 중간에 얼마나 오래 멈춰도 발화가 끝난 것으로 보지 않을지를 정합니다(0.3~2.0초, 기본 0.5초). 생각하거나 숫자를 받아쓸 때 끊긴다면 늘리세요. 이 값은 파이프라인을 만들 때 반영되므로 오버레이가 실행 중일 때 바꾸면 모델을 다시 불러옵니다.

**받아쓰기 정리**(실험적, 기본 꺼짐)는 온디바이스 LLM으로 전사 결과의 문장부호를
고치고 군말을 제거합니다. 처음 사용할 때 SmolLM2-360M-Instruct(374 MB,
Apache-2.0)를 내려받습니다. 모델 출력이 실제 말한 내용에서 벗어나면(없는 내용을
만들거나 정리 대신 대답하는 경우) 원본 전사를 그대로 넣습니다. 버블을 길게 누르면
설정을 켜지 않고 그 한 번만 정리할 수 있습니다.

설정 화면에서 **음성 모델**도 고를 수 있습니다 — Parakeet TDT(114개 언어,
스트리밍 부분 결과) 또는 Whisper Small(4개 언어, 발화가 끝난 뒤 디코딩,
더 작은 다운로드). 또한 휴대폰 대신 **블루투스 헤드셋**으로 녹음할 수
있습니다. 헤드셋 마이크는 대역이 좁은 경우가 많아 기본은 꺼짐이며, 헤드셋이
없으면 휴대폰 마이크로 되돌아갑니다.
