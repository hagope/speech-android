...
设置界面的**停顿容忍度**决定你在句中可以停顿多久才会结束这段语音 —— 0.3–2.0 秒,默认 0.5 秒。若你在思考或口述数字时被打断,可以调高它。该值在构建管线时写入,因此在悬浮窗运行期间修改会重新加载模型。

**清理听写文本**(实验性,默认关闭)会用设备端 LLM 处理转录文本,修正标点并
移除语气词,使用首次启用时下载的 SmolLM2-360M-Instruct(374 MB,Apache-2.0)。
若模型输出偏离你实际说的内容(凭空添加文字,或直接作答而非整理),则原样插入
未处理的转录文本。长按悬浮按钮可仅对本次听写进行清理,而无需开启该设置。

设置界面还可选择**语音模型** —— Parakeet TDT(114 种语言,流式部分结果)或
Whisper Small(4 种语言,每段语音结束后解码,下载更小) —— 并可改用
**蓝牙耳机**录音而非手机麦克风。耳机麦克风带宽通常较窄,因此该项默认关闭,
未连接耳机时会回退到手机麦克风。
