...
**Допустимая пауза** на экране настройки задаёт, как долго можно молчать
посреди фразы, прежде чем реплика будет завершена: 0,3–2,0 с, по умолчанию 0,5
с. Увеличьте её, если вас обрывает во время раздумий или диктовки цифр.
Значение фиксируется при создании конвейера, поэтому изменение при работающем
оверлее перезагружает модели.

**Очистка диктовки** (экспериментально, по умолчанию выключено) прогоняет
расшифровку через локальную LLM, исправляя пунктуацию и убирая слова-паразиты;
SmolLM2-360M-Instruct (374 МБ, Apache-2.0) загружается при первом включении.
Если вывод модели расходится со сказанным — выдуманный текст или ответ вместо
приведения в порядок — вставляется исходная расшифровка без изменений.
Удерживайте кнопку, чтобы очистить одну диктовку, не включая настройку.

На экране настройки также выбирается **модель распознавания** — Parakeet TDT
(114 языков, потоковые частичные результаты) или Whisper Small (4 языка,
расшифровка после каждой реплики, меньше загрузка) — и можно записывать с
**Bluetooth-гарнитуры** вместо телефона. Микрофоны гарнитур обычно уже по
полосе, поэтому это выключено по умолчанию и возвращается к микрофону
телефона, если гарнитура не подключена.
